...
public int CrearMetodoPago (MetodoPagoEN metodoPago)
{
        ...
        try
        {
                ...
                if (metodoPago.UsuarioPoseedor != null) {
                        for (int i = 0; i < metodoPago.UsuarioPoseedor.Count; i++) {
                                metodoPago.UsuarioPoseedor [i] = (NemesisNevulaGen.ApplicationCore.EN.NemesisNevula.UsuarioEN)session.Load (typeof(NemesisNevulaGen.ApplicationCore.EN.NemesisNevula.UsuarioEN), metodoPago.UsuarioPoseedor [i].Id);
                                metodoPago.UsuarioPoseedor [i].MetodoPago.Add (metodoPagoNH);
                        }
                }
