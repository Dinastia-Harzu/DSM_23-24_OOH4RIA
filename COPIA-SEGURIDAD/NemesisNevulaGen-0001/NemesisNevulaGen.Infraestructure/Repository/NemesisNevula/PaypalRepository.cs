...
public int CrearPaypal (PaypalEN paypal)
{
        ...
        try
        {
                ...
                if (paypal.UsuarioPoseedor != null) {
                        for (int i = 0; i < paypal.UsuarioPoseedor.Count; i++) {
                                paypal.UsuarioPoseedor [i] = (NemesisNevulaGen.ApplicationCore.EN.NemesisNevula.UsuarioEN)session.Load (typeof(NemesisNevulaGen.ApplicationCore.EN.NemesisNevula.UsuarioEN), paypal.UsuarioPoseedor [i].Id);
                                paypal.UsuarioPoseedor [i].MetodoPago.Add (paypalNH);
                        }
                }
