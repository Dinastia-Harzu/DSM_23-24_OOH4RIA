...
public void DevolverArticulo (int p_Usuario_OID, int p_compraUsuario_OIDs)
{
        ...
        try
        {
                ...
                // Eliminamos el articulo del usuario
                IList<int> artsQuitados = new List<int>
                {
                        compra.Articulo.Id
                };

                Console.WriteLine (compra.Articulo.Id);

                usuarioCEN.QuitarArticulo (p_Usuario_OID, artsQuitados);

                compraCEN.get_ICompraRepository ().BorrarCompra (p_compraUsuario_OIDs);   // Borramos la compra

                usuarioCEN.get_IUsuarioRepository ().ModificarUsuario (usuario);  // Modificamos el usuario con el dinero reembolsado
                CPSession.Commit ();
        }
        catch (Exception ex)
        {
